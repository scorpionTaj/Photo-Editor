...
root = Tk()
root.title("Scorpion Photo Editor")
root.geometry("800x700")

# Custom Fonts
font_style = ("JetBrains Mono", 12)
font_bold = ("JetBrains Mono", 12, "bold")
...
Label(root, text="Blur:", font=font_bold).place(x=15, y=10)
v1 = IntVar()
Scale(root, from_=0, to=10, variable=v1, orient=HORIZONTAL, command=blur).place(x=150, y=10)

Label(root, text="Brightness:", font=font_bold).place(x=8, y=55)
v2 = IntVar()
Scale(root, from_=0, to=10, variable=v2, orient=HORIZONTAL, command=adjust_brightness).place(x=150, y=55)

Label(root, text="Contrast:", font=font_bold).place(x=35, y=100)
v3 = IntVar()
Scale(root, from_=0, to=10, variable=v3, orient=HORIZONTAL, command=adjust_contrast).place(x=150, y=100)

Label(root, text="Rotate:", font=font_bold).place(x=370, y=8)
values = [0, 90, 180, 270, 360]
rotate_combo = ttk.Combobox(root, values=values, font=font_style)
rotate_combo.place(x=460, y=15)
rotate_combo.bind("<<ComboboxSelected>>", rotate_image)

Label(root, text="Flip:", font=font_bold).place(x=400, y=50)
values1 = ["FLIP LEFT TO RIGHT", "FLIP TOP TO BOTTOM"]
flip_combo = ttk.Combobox(root, values=values1, font=font_style)
flip_combo.place(x=460, y=57)
flip_combo.bind("<<ComboboxSelected>>", flip_image)

Label(root, text="Add border:", font=font_bold).place(x=320, y=92)
values2 = [i for i in range(10, 45, 5)]
border_combo = ttk.Combobox(root, values=values2, font=font_style)
border_combo.place(x=460, y=99)
border_combo.bind("<<ComboboxSelected>>", add_border)
...
# Buttons
Button(root, text="Select Image", bg='black', fg='gold', font=font_bold, relief=GROOVE, command=select_image).place(x=100, y=595)
Button(root, text="Save", width=12, bg='black', fg='gold', font=font_bold, relief=GROOVE, command=save_image).place(x=280, y=595)
Button(root, text="Exit", width=12, bg='black', fg='gold', font=font_bold, relief=GROOVE, command=root.destroy).place(x=460, y=595)
